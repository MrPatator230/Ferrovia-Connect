 ...
 * Implémentation cohérente avec api_quais.php (PDO, mêmes identifiants)
 *
...
// Configuration DB (reprend la configuration utilisée dans api_quais.php)
$host = 'localhost';
$port = 3306;
...
try {
    switch ($action) {
        ...
        default:
            // default behaviour: return Bourgogne-Franche-Comté list if no action
            if ($action === '') {
...
function getByRegion(PDO $pdo, string $region) {
    ...
    if ($hasRegion) {
        ...
        // fallback: search title/content for the region keyword
        $like = '%' . $region . '%';
